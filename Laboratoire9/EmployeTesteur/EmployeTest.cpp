...
public:
	EmployeParam() :
			employe("test", "test", Date(14, 6, 1987), 14) {
	}
...
protected:
	...
	virtual void SetUp() {
		nom = "Levasseur";
		prenom = "Pierre-Marc";
		dateNaissance = Date(31, 10, 1990);
		codeDepartement = 12;
		employe = EmployeImpl(nom, prenom, dateNaissance, codeDepartement);
	}
...
TEST_F(EmployeParam, reqEmployeFormate) {
	string chaineFormateAttendue = prenom + " " + nom;

	ASSERT_EQ(chaineFormateAttendue, employe.reqEmployeFormate());
}
